...
summary: "Learn about the Oracle Database 26ai high availability solution architecture on Google Cloud NetApp Volumes, including reference data paths, platform responsibilities, and storage topology."
---
= Solution architecture for Oracle Database 26ai high availability on Google Cloud NetApp Volumes
:icons: font
:imagesdir: ../media/

[.lead]
Learn about the Oracle Database 26ai high availability architecture on Google Cloud NetApp Volumes, including reference data paths, platform responsibilities, and storage topology for primary, standby, and observer components.

== Architecture overview
This architecture deploys *Oracle Database 26ai* HA on Google Cloud using *Google Cloud NetApp Volumes (GCNV)* iSCSI storage and *Oracle Data Guard*. 
...
.Zoning
Deploy one GCNV Flex Unified pool per database zone and assign dedicated volumes to each host. Use boot disks for the operating system only. *Out of scope:* TDE, RAC, NVMe/TCP, OEM, and Active Data Guard (the standby remains *MOUNTED*).
...
== What's next?
To choose the deployment path that matches your recovery and availability goals, continue with xref:gcnv-oracle-deployment-options.adoc[Learn about deployment options for Oracle Database 26ai on Google Cloud NetApp Volumes].
