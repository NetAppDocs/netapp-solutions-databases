...
[.lead]
Create the Oracle primary database on Google Cloud NetApp Volumes iSCSI storage using Oracle Database Configuration Assistant in silent mode. This procedure covers running `dbca` to create the container database and pluggable database on ASM, configuring archive log destinations, and adding a role-based application service for transparent failover after Data Guard is enabled.
...
== What's next?
To establish standby protection and readiness for failover, proceed to xref:gcnv-oracle-standby-database.adoc[Create the Oracle standby database] on `oracdb2`.
