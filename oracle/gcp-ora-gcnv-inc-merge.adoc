...
image:gcp-ora-gcnv-inc-merge-architecture.png["This image provides a detailed picture of the Oracle VLDB incremental merge implementation in Google cloud with GCNV."]
...
+
image:gcp-ora-gcnv-inc-merge-pool-001.png[This image showcases GCNV storage pool screen from Google cloud portal console]
...
+
image:gcp-ora-gcnv-inc-merge-vol-002.png[This image showcases GCNV storage volumes screen from Google cloud portal console]
...
+
image:gcp-ora-gcnv-inc-merge-vol-003.png[This image showcases GCNV storage volumes screen from Google cloud portal console]
...
Following automation toolkit contains a playbook that can be used to create or refresh clones of an Oracle VLDB from a primary database backup image copy stored on GCNV storage via NFS mount for a complete clone lifecycle management. For details, referred to documentation: link:automation-ora-bkup-clone.html[Automated Oracle VLDB Clone Lifecycle with In-server image backup].
...
// NetApp Solutions restructuring (jul 2025) - renamed from databases/gcp_ora_gcnv_inc_merge.adoc
