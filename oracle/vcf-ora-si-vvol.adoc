...
image:vcf-ora-si-vvols-architecture.png["This image provides a detailed picture of the Oracle deployment configuration in AWS public cloud with iSCSI and ASM."]
...
+
image:vcf-ora-vvol-datastore-002.png["Screenshot showing vVols datastore configuration."]
...
+
image:vcf-ora-vvol-datastore-003.png["Screenshot showing vVols datastore configuration."]

. Choose `Create new volumes`, fill in the volume name and size and click on `ADD` then `NEXT` to move to the summary page. 
+
image:vcf-ora-vvol-datastore-004.png["Screenshot showing vVols datastore configuration."]
+
[NOTE]
...
+
image:vcf-ora-vvol-datastore-005.png["Screenshot showing vVols datastore configuration."]

. Repeat the same procedures to create vVols datastore for Oracle data and log. 
+
image:vcf-ora-vvol-datastore-006.png["Screenshot showing vVols datastore configuration."]
...
+
image:vcf-ora-vvol-vm-policy-005.png["Screenshot showing a VM storage policy configuration."]
...
+
image:vcf-ora-vvol-vm-bins.png["Screenshot showing a VM storage configuration."]

. Add a disk to VM for Oracle data storage.
+
image:vcf-ora-vvol-vm-data.png["Screenshot showing a VM storage configuration."]

. Add a disk to VM for Oracle log storage.
+
image:vcf-ora-vvol-vm-logs.png["Screenshot showing a VM storage configuration."]
...
. Configure deployment target file - hosts, global variables file - vars/vars.yml, and local DB VM variables file - host_vars/host_name.yml according to instructions in this section of TR-4992: link:../automation/automation-ora-c-series-nfs.html#parameter-files-configuration[Parameter files configuration^]. Comment out nfs_lif variable from local DB VM variable file.
...
+
image:vcf-ora-vvol-em-002.png["Screenshot showing Oracle EM Express configuration."]
image:vcf-ora-vvol-em-001.png["Screenshot showing Oracle EM Express configuration."]
...
+
image:vcf-ora-vvol-snapctr-deploy-008.png["Screenshot showing SnapCenter configuration."]
image:vcf-ora-vvol-snapctr-deploy-009.png["Screenshot showing SnapCenter configuration."]
image:vcf-ora-vvol-snapctr-deploy-010.png["Screenshot showing SnapCenter configuration."]
...
image:vcf-ora-vvol-snapctr-deploy-012.png["Screenshot showing SnapCenter configuration."]
...
+
image:vcf-ora-vvol-snapctr-bkup-001.png["Screenshot showing SnapCenter configuration."]

. Click on database to start a workflow to enable protection for the database.
+
image:vcf-ora-vvol-snapctr-bkup-002.png["Screenshot showing SnapCenter configuration."]

. Apply backup policy, setup scheduling if needed. 
+
image:vcf-ora-vvol-snapctr-bkup-003.png["Screenshot showing SnapCenter configuration."]

. Setup backup job notification if required.
+
image:vcf-ora-vvol-snapctr-bkup-005.png["Screenshot showing SnapCenter configuration."]

. Review the summary and finish to enable database protection.
+
image:vcf-ora-vvol-snapctr-bkup-006.png["Screenshot showing SnapCenter configuration."]

. On-demand backup job can be triggered by click on `Back up Now`.
+
image:vcf-ora-vvol-snapctr-bkup-007.png["Screenshot showing SnapCenter configuration."]
image:vcf-ora-vvol-snapctr-bkup-008.png["Screenshot showing SnapCenter configuration."]

. The backup job can be monitored at the `Monitor` tab by clicking on the running job.
+
image:vcf-ora-vvol-snapctr-bkup-009.png["Screenshot showing SnapCenter configuration."]
...
+
image:vcf-ora-vvol-snapctr-restore-001.png["Screenshot showing SnapCenter configuration."]

. Choose restore scope.
+
image:vcf-ora-vvol-snapctr-restore-002.png["Screenshot showing SnapCenter configuration."]

. Choose recovery scope to `All Logs`. 
+
image:vcf-ora-vvol-snapctr-restore-003.png["Screenshot showing SnapCenter configuration."]

. Specify any optional pre-scripts to run.
+
image:vcf-ora-vvol-snapctr-restore-004.png["Screenshot showing SnapCenter configuration."]

. Specify any optional after-script to run.
+
image:vcf-ora-vvol-snapctr-restore-005.png["Screenshot showing SnapCenter configuration."]

. Send a job report if desired.
+
image:vcf-ora-vvol-snapctr-restore-006.png["Screenshot showing SnapCenter configuration."]

. Review the summary and click on `Finish` to launch the restoration and recovery.
+
image:vcf-ora-vvol-snapctr-restore-007.png["Screenshot showing SnapCenter configuration."]

. From `Monitor` tab, open the job to review the details.
+
image:vcf-ora-vvol-snapctr-restore-008.png["Screenshot showing SnapCenter configuration."]
...
+
image:vcf-ora-vvol-snapctr-clone-001.png["Screenshot showing SnapCenter configuration."]

. Name the clone database SID.
+
image:vcf-ora-vvol-snapctr-clone-002.png["Screenshot showing SnapCenter configuration."]
...
+
image:vcf-ora-vvol-snapctr-clone-003.png["Screenshot showing SnapCenter configuration."]

. Select the proper ORACLE_HOME, user and group on the target host. Keep credential at default.
+
image:vcf-ora-vvol-snapctr-clone-004.png["Screenshot showing SnapCenter configuration."]

. You may change clone database parameters to meet configuration or resources requirements for the clone database.
+
image:vcf-ora-vvol-snapctr-clone-005.png["Screenshot showing SnapCenter configuration."]

. Choose recovery scope. `Until Cancel` recovers the clone up to last available log file in the backup set. 
+
image:vcf-ora-vvol-snapctr-clone-006.png["Screenshot showing SnapCenter configuration."]

. Review the summary and launch the clone job.
+
image:vcf-ora-vvol-snapctr-clone-008.png["Screenshot showing SnapCenter configuration."]

. Monitor the clone job execution from `Monitor` tab.
+
image:vcf-ora-vvol-snapctr-clone-009.png["Screenshot showing SnapCenter configuration."]

. Cloned database is immediately registered in SnapCenter.
+
image:vcf-ora-vvol-snapctr-clone-010.png["Screenshot showing SnapCenter configuration."]
...
// NetApp Solutions restructuring (jul 2025) - renamed from databases/vcf_ora_si_vvol.adoc
