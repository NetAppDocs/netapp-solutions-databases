...
| orap - primary DB server | NTAP(NTAP_PDB1,NTAP_PDB2,NTAP_PDB3) | /u01, /u02, /u03 NFS mounts on Google NetApp Volumes storage pool
| orap2 - restored from backup | NTAP(NTAP_PDB1,NTAP_PDB2,NTAP_PDB3) | /u01, /u02, /u03 NFS mounts on Google NetApp Volumes storage pool
|===
...
=== Oracle database backup in Google Cloud with Google NetApp Volumes
[%collapsible%close]
...
. Read the README file in the toolkit for detailed instructions on how to configure and schedule the backup job from the crontab or any other scheduling tool. The playbook is designed to be run on Ansible controller node with access to Oracle DB server VMs and Google NetApp Volumes. It will create snapshots of the database volumes based on the defined schedule and retention policy, and copy the snapshots to vault for disaster recovery purposes. 

. By default, the playbook creates a daily snapshot backup and an hourly snapshots every hour.  The default snapshots retention is 7 daily snapshots and 24 hourly snapshots. Any additional snapshots exceeding retention are pruned and maintain a rolling copies of 7 daily snapshots and 24 hourly snapshots. You can adjust the backup frequency and retention based on your RTO/RPO requirements and storage cost consideration. While daily snapshot backup all DB volumes, hourly snapshot backup only log volumes and save storage space.
...
. The weekly vault backup purge all existing backups and reset the baseline. Once baseline backup is established. All additional vault backup are incremental using incremental forever methodology and all vault backup are created from most recent app-consistent snapshots at the time of execution to ensure recoverbility. In above example, a full backup and baseline reset happens on every sunday when weekly vault backup job run at 12:05 am. The daily vault backup script creates a incremental backup copy of the database from daily snapshot from Monday to Saturday at 12:05 am. You can adjust the schedule as needed. The script can also be executed at hourly basis if you want to have more frequent vault backup.
...
=== Oracle database recovery and clone in Google Cloud with Google NetApp Volumes
[%collapsible%close]

==== Oracle database in-place, point-in-time recovery using Google NetApp Volumes snapshots
[%collapsible%close]
====
...
. Stop Oracle services to shutdown Oracle database and unmount the file system on the host before restoring from a snapshot.
+
....
[root@orap admin]# systemctl stop oracle_NTAP
[root@orap admin]# ps -ef | grep ora
root        1410       1  0 Feb04 ?        00:00:09 /opt/google-cloud-ops-agent/libexec/google_cloud_ops_agent_wrapper -config_path /etc/google-cloud-ops-agent/config.yaml -log_path /var/log/google-cloud-ops-agent/subagents/logging-module.log /opt/google-cloud-ops-agent/subagents/fluent-bit/bin/fluent-bit --config /run/google-cloud-ops-agent-fluent-bit/fluent_bit_main.conf --parser /run/google-cloud-ops-agent-fluent-bit/fluent_bit_parser.conf --storage_path /var/lib/google-cloud-ops-agent/fluent-bit/buffers
root        1714    1410  0 Feb04 ?        00:04:21 /opt/google-cloud-ops-agent/subagents/fluent-bit/bin/fluent-bit --config /run/google-cloud-ops-agent-fluent-bit/fluent_bit_main.conf --parser /run/google-cloud-ops-agent-fluent-bit/fluent_bit_parser.conf --storage_path /var/lib/google-cloud-ops-agent/fluent-bit/buffers
root       61557   61354  0 20:45 pts/0    00:00:00 grep --color=auto ora
...
. After the recovery is completed, you need to perform additional steps such as revising listener.ora, tnsnames.ora files to match with new hostname or IP address. If needed, setup systemd service to shutdown and restart database to complete the recovery process.

====

==== Oracle database cloning to a new host using Google NetApp Volumes snapshots or vault backups
[%collapsible%close]
====

The Oracle database cloning to a new host using Google NetApp Volumes snapshots or vault backups is identical to the previous section describes how to use Google NetApp Volumes snapshots or vault backups to restore and recovery of an Oracle database on a new host in the event of a failure. But renaming the cloned database may be only additional steps desired, which can be easiy done with Oracle utilities. The DB clone can be useful for UAT testing, development, or other purposes.

For some customers, who needs automated cloning and refresh, please raise a request to NetApp Solutions team for a sample Ansible playbook that can be used as a reference for automating the cloning and refresh process with Google NetApp Volumes snapshots or vault backups. Following is the link for put in a request to NetApp Solutions Engineering team:link:https://github.com/NetAppDocs/netapp-solutions-family/issues/new?labels=databases,automation&title=Request%20Automation%20-%20%3Crequest_title%3E&body==%0A%0A%20Solution%20Name:%20%0A%20Request%20Title:%20%0A%20Description:%0A%20Suggestions:%0A[Request for Automation]
