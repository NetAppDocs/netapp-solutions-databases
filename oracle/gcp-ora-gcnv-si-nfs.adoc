...
* Automated Oracle database deployment on Google Cloud NetApp Volumes in Google Cloud with NFS.  
...
The Oracle database cloning to a new host using Google Cloud NetApp Volumes snapshots or vault backups is identical to the previous section describes how to use Google Cloud NetApp Volumes snapshots or vault backups to restore and recovery of an Oracle database on a new host in the event of a failure. But renaming the cloned database may be only additional steps desired, which can be easiy done using Oracle dbnewid utility. The DB clone can be useful for UAT testing, development, or other purposes.
