...
image:vcf-orarac-vvol-architecture.png["This image provides a detailed picture of the Oracle deployment configuration in AWS public cloud with iSCSI and ASM."]
...
+
image:vcf-ora-vvol-scp-001.png["Screenshot showing a custom storage capability profile configuration."]
image:vcf-ora-vvol-scp-008.png["Screenshot showing a custom storage capability profile configuration."]

. Click on `Storage capability profile` to add a custom profile for Oracle. Name the profile and add a brief description.
+
image:vcf-ora-vvol-scp-002.png["Screenshot showing a custom storage capability profile configuration."]

. Choose storage controller category: performance, capacity, or hybrid.
+
image:vcf-ora-vvol-scp-003.png["Screenshot showing a custom storage capability profile configuration."]

. Select the protocol.
+
image:vcf-ora-vvol-scp-004.png["Screenshot showing a custom storage capability profile configuration."]

. Define a QoS policy if desired.
+
image:vcf-ora-vvol-scp-005.png["Screenshot showing a custom storage capability profile configuration."]
...
+
image:vcf-ora-vvol-scp-006.png["Screenshot showing a custom storage capability profile configuration."]

. Review the summary and finish the storage capability profile creation.
+
image:vcf-ora-vvol-scp-007.png["Screenshot showing a custom storage capability profile configuration."]
...
+
image:vcf-ora-vvol-datastore-001.png["Screenshot showing vVols datastore configuration."]
+
image:vcf-ora-vvol-datastore-001-b.png["Screenshot showing vVols datastore configuration."]

. Filling in general information for datastore including provisioning destination, type, name, and protocol.
+
image:vcf-orarac-vvol-datastore-001.png["Screenshot showing vVols datastore configuration."]
...
+
image:vcf-orarac-vvol-datastore-002.png["Screenshot showing vVols datastore configuration."]

. Choose `Create new volumes`, fill in the volume name and size and click on `ADD` then `NEXT` to move to the summary page. 
+
image:vcf-orarac-vvol-datastore-003.png["Screenshot showing vVols datastore configuration."]
image:vcf-orarac-vvol-datastore-004.png["Screenshot showing vVols datastore configuration."]

. Click `Finish` to create vVols datastore for Oracle binary. 
+
image:vcf-orarac-vvol-datastore-005.png["Screenshot showing vVols datastore configuration."]

. Create datastore for Oracle cluster registry or CRS.  
+
image:vcf-orarac-vvol-datastore-006.png["Screenshot showing vVols datastore configuration."]
+
[NOTE]
...
+
image:vcf-orarac-vvol-datastore-006-a.png["Screenshot showing vVols datastore configuration."]
image:vcf-orarac-vvol-datastore-006-b.png["Screenshot showing vVols datastore configuration."]
...
+
image:vcf-orarac-vvol-datastore-006-c.png["Screenshot showing vVols datastore configuration."]

. Validate the Oracle datastores after deployment.
+
image:vcf-orarac-vvol-datastore-007.png["Screenshot showing vVols datastore configuration."]
...
+
image:vcf-ora-vvol-vm-policy-001.png["Screenshot showing a VM storage policy configuration."]

. Name the VM storage policy.
+
image:vcf-ora-vvol-vm-policy-002.png["Screenshot showing a VM storage policy configuration."]

. In `Datastore specific rules`, check `Enable rules for "NetAPP.clustered.Data.ONTAP.VP.vvol" storage`
+
image:vcf-ora-vvol-vm-policy-003.png["Screenshot showing a VM storage policy configuration."]
...
+
image:vcf-ora-vvol-vm-policy-004.png["Screenshot showing a VM storage policy configuration."]

. For NetApp.clustered.Data.ONTAP.VP.vvol rules `Replication`, choose `Disabled` if vVols are not replicated.
+
image:vcf-ora-vvol-vm-policy-004-a.png["Screenshot showing a VM storage policy configuration."]

. Storage compatibility page displays the compatible vVols datastores in VCF environment.
+
image:vcf-orarac-vvol-datastore-008.png["Screenshot showing a VM storage policy configuration."]

. Review and finish to create the VM Storage Policy.
+
image:vcf-ora-vvol-vm-policy-006.png["Screenshot showing a VM storage policy configuration."]

. Validate the VM Storage Policy just created. 
+
image:vcf-ora-vvol-vm-policy-007.png["Screenshot showing a VM storage policy configuration."]
...
+
image:vcf-orarac-vvol-vm-001.png["Screenshot showing a VM storage configuration."]

. From RAC node 1 - ora01, add a disk to VM for Oracle binary storage without sharing.
+
image:vcf-orarac-vvol-vm-002.png["Screenshot showing a VM storage configuration."]

. From RAC node 1, add three disks to VM for Oracle RAC CRS storage and enable multi-writer sharing.
+
image:vcf-orarac-vvol-vm-003.png["Screenshot showing a VM storage configuration."]
image:vcf-orarac-vvol-vm-004.png["Screenshot showing a VM storage configuration."]
image:vcf-orarac-vvol-vm-005.png["Screenshot showing a VM storage configuration."]

. From RAC node 1, add two disks each from each datastore for data to VM for shared Oracle data storage.
+
image:vcf-orarac-vvol-vm-006.png["Screenshot showing a VM storage configuration."]
image:vcf-orarac-vvol-vm-008.png["Screenshot showing a VM storage configuration."]
image:vcf-orarac-vvol-vm-009.png["Screenshot showing a VM storage configuration."]
image:vcf-orarac-vvol-vm-010.png["Screenshot showing a VM storage configuration."]

. From RAC node 1, add two disks to VM from logs datastore for shared Oracle log files storage.
+
image:vcf-orarac-vvol-vm-011.png["Screenshot showing a VM storage configuration."]
image:vcf-orarac-vvol-vm-012.png["Screenshot showing a VM storage configuration."]

. From RAC node 2, add a disk to VM for Oracle binary storage without sharing.
+
image:vcf-orarac-vvol-vm-013.png["Screenshot showing a VM storage configuration."]
...
+
image:vcf-orarac-vvol-vm-014.png["Screenshot showing a VM storage configuration."]
image:vcf-orarac-vvol-vm-015.png["Screenshot showing a VM storage configuration."]
...
+
image:vcf-ora-vvol-vm-uuid.png["Screenshot showing a VM storage configuration."]
...
+
image:vcf-orarac-vvol-em-001.png["Screenshot showing Oracle EM Express configuration."]
image:vcf-orarac-vvol-em-002.png["Screenshot showing Oracle EM Express configuration."]
...
+
image:vcf-ora-vvol-snapctr-prerequisites.png["Screenshot showing SnapCenter 6.0 prerequisites."]
...
+
image:vcf-ora-vvol-snapctr-deploy-001.png["Screenshot showing SnapCenter configuration."]

. Review `Get Started` menu to get up to speed on SnapCenter if you are a new user.
+
image:vcf-ora-vvol-snapctr-deploy-002.png["Screenshot showing SnapCenter configuration."]

. Update `Hypervisor Settings` in global settings.
+
image:aws-ora-fsx-vmc-snapctr-001.png["Screenshot showing SnapCenter configuration."]

. Add ONTAP storage cluster to `Storage Systems` with cluster management IP and authenticated via cluster admin user ID. 
+
image:vcf-ora-vvol-snapctr-deploy-006.png["Screenshot showing SnapCenter configuration."]
image:vcf-ora-vvol-snapctr-deploy-007.png["Screenshot showing SnapCenter configuration."]
...
+
image:aws-ora-fsx-vmc-snapctr-003.png["Screenshot showing SnapCenter configuration."]

. Add Oracle RAC database VM in VCF to `Hosts` with DB VM credential created in previous step.  
+
image:vcf-orarac-vvol-snapctr-setup-001.png["Screenshot showing SnapCenter configuration."]
image:vcf-orarac-vvol-snapctr-setup-002.png["Screenshot showing SnapCenter configuration."]
image:vcf-orarac-vvol-snapctr-setup-003.png["Screenshot showing SnapCenter configuration."]

. Similarly, add NetApp VMware plugin VM to `Hosts` with vSphere plugin VM credential created in previous step.
+
image:vcf-ora-vvol-snapctr-deploy-011.png["Screenshot showing SnapCenter configuration."]
image:vcf-orarac-vvol-snapctr-setup-004.png["Screenshot showing SnapCenter configuration."]
...
+
image:aws-ora-fsx-vmc-snapctr-002.png["Screenshot showing SnapCenter configuration."]
...
+
image:vcf-orarac-vvol-snapctr-bkup-001.png["Screenshot showing SnapCenter configuration."]

. Click on database to start a workflow to enable protection for the database.
+
image:vcf-orarac-vvol-snapctr-bkup-002.png["Screenshot showing SnapCenter configuration."]

. Apply backup policy, setup scheduling if needed. 
+
image:vcf-orarac-vvol-snapctr-bkup-003.png["Screenshot showing SnapCenter configuration."]

. Setup backup job notification if required.
+
image:vcf-orarac-vvol-snapctr-bkup-005.png["Screenshot showing SnapCenter configuration."]

. Review the summary and finish to enable database protection.
+
image:vcf-orarac-vvol-snapctr-bkup-006.png["Screenshot showing SnapCenter configuration."]

. On-demand backup job can be triggered by click on `Back up Now`.
+
image:vcf-orarac-vvol-snapctr-bkup-007.png["Screenshot showing SnapCenter configuration."]
image:vcf-orarac-vvol-snapctr-bkup-008.png["Screenshot showing SnapCenter configuration."]

. The backup job can be monitored at the `Monitor` tab by clicking on the running job.
+
image:vcf-orarac-vvol-snapctr-bkup-009.png["Screenshot showing SnapCenter configuration."]

. Click on database to review the backup sets completed for RAC database. 
+
image:vcf-ora-vvol-snapctr-bkup-010.png["Screenshot showing SnapCenter configuration."]
...
+
image:vcf-orarac-vvol-snapctr-restore-001.png["Screenshot showing SnapCenter configuration."]

. Choose restore scope.
+
image:vcf-orarac-vvol-snapctr-restore-002.png["Screenshot showing SnapCenter configuration."]

. Choose recovery scope to `All Logs`. 
+
image:vcf-orarac-vvol-snapctr-restore-003.png["Screenshot showing SnapCenter configuration."]

. Specify any optional pre-scripts to run.
+
image:vcf-orarac-vvol-snapctr-restore-004.png["Screenshot showing SnapCenter configuration."]

. Specify any optional after-script to run.
+
image:vcf-orarac-vvol-snapctr-restore-005.png["Screenshot showing SnapCenter configuration."]

. Send a job report if desired.
+
image:vcf-orarac-vvol-snapctr-restore-006.png["Screenshot showing SnapCenter configuration."]

. Review the summary and click on `Finish` to launch the restoration and recovery.
+
image:vcf-orarac-vvol-snapctr-restore-007.png["Screenshot showing SnapCenter configuration."]
...
// NetApp Solutions restructuring (jul 2025) - renamed from databases/vcf_ora_rac_vvol.adoc
