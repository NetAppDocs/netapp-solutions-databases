...
*To use only part of this guide, stop at the section that matches your tier:*

* *Dev/Test or Prod Basic (Restart):* Follow <<prerequisites,Prerequisites>> through <<create-primary-database,Create the primary database>>.
* *Prod HA (no Data Guard):* Complete the above, then add <<gcnv-standby-initialization,Step 3: GCNV standby initialization>> for storage-layer seeding.
* *Prod HA (Data Guard + FSFO):* Follow the full guide.
