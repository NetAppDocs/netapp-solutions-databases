...
| Yes
| No
| <<prerequisites,Prerequisites>> through <<provision-gcnv-iscsi-volumes,Step 2: Provision GCNV iSCSI volumes>>, <<install-oracle-db-home,Install Oracle Database 26ai on each DB host>>, and <<create-primary-database,Create the primary database>>
| Snapshot clone
...
| Yes
| Partial
| <<prerequisites,Prerequisites>> through <<create-primary-database,Create the primary database>>; see <<gcnv-standby-initialization,GCNV standby initialization>> for GCNV seed/clone
| GCNV clone recovery
...
== Step 1: Provision Compute Engine
...
Enable *Secure Boot*, *vTPM*, and *Integrity Monitoring* on all three. The boot disk holds the *OS only*. `/u01`, *Grid/DB homes, staging, and all ASM data* use *GCNV iSCSI volumes* (see <<provision-gcnv-iscsi-volumes,Step 2: Provision GCNV iSCSI volumes>>) — do *not* attach a separate GCE data disk for `/u01`.

[[vpc-firewall-allow-1521]]
=== VPC firewall — allowlist TCP/1521 across all three zones
...
| `1521` | Connection refused | Firewall open; Grid listener starts during <<install-grid-infrastructure,Install Oracle Grid Infrastructure (Oracle Restart) on each DB host>>
...
Run on `oracdb1` *and* `oracdb2` (Observer setup is covered in <<install-instant-client-on-observer,Install Oracle Instant Client on the Observer host>>).
...
=== Capture the iSCSI initiator name (IQN)

After the reboot, capture the IQN of each DB host. You will use these IQNs to create the GCNV host groups in <<create-host-groups,Create the host groups>>.
...
== Step 2: Provision GCNV iSCSI volumes

[[create-flex-unified-pool-per-zone]]
=== Create one GCNV Flex Unified iSCSI storage pool per database zone
...
Wait for `READY`. Scale pool sizes to your database footprint (the sizes in <<create-gcnv-iscsi-volumes,Create the GCNV iSCSI volumes>> are examples).

*Default-mode (this guide):* Flex Unified pools use *Default-mode* (`--mode=default`). Create pools and iSCSI volumes with Cloud Console or `gcloud netapp`. *Volume replication*, snapshots, and clones use Google Cloud APIs (<<gcnv-standby-initialization,GCNV standby initialization>>).

[[create-host-groups]]
=== Create the host groups (one per DB host)
...
=== Create the GCNV iSCSI volumes (per database host)
...
*Minimal layout (validation only):* Two LUNs per host (`*_data`, `*_reco`) with `arch_01p1`→`+RECO` and `arch_01p2`→`+FRA` is acceptable for lab; *production uses five volumes per <<create-gcnv-iscsi-volumes,Create the GCNV iSCSI volumes>>.*
...
* *Volume serial* from the Cloud Console — use with host-discovered WWID in <<configure-iscsi-and-multipath,Configure Linux iSCSI and multipath for GCNV iSCSI volumes>>

[[configure-iscsi-and-multipath]]
=== Configure Linux iSCSI and multipath for GCNV iSCSI volumes

Run on `oracdb1` using that host’s pool portals, then on `oracdb2` using the standby pool portals.

If host *egress* is restricted, allow *TCP/3260* from each database VM to its GCNV iSCSI portal IPs (in addition to inter-VM TCP/1521 from <<vpc-firewall-allow-1521,VPC firewall — allowlist TCP/1521 across all three zones>>).
...
=== Partition ASM backing devices on GCNV iSCSI volumes
...
=== Format and mount `/u01` on the local GCNV iSCSI volume
...
Reboot once and confirm `/u01` mounts before <<install-oracle-software,Step 3: Install Oracle software>>.
...
=== Install Oracle Grid Infrastructure (Oracle Restart) on each DB host
...
=== Install Oracle Database 26ai on each DB host

Run this section on `oracdb1`, then on `oracdb2`. The standby database is created in <<create-standby-database,Step 4: Create the standby database>>.
...
(Use `ORACLE_SID=orcls` on the standby host. The standby database is created in <<create-standby-database,Step 4: Create the standby database>>.)
...
Point archivelogs at `+RECO` (the standby uses matching settings in <<standby-init-ora-and-nomount,Standby: minimal init.ora pfile, password file, NOMOUNT>>):
...
=== Standby: minimal init.ora pfile, password file, NOMOUNT
...
NOMOUNT with no datafiles until <<gcnv-standby-initialization,GCNV standby initialization>>.
...
Populate standby ASM volumes in *`oracle-pool-b`* using *Google Cloud NetApp Volumes replication* (SnapMirror-based, *Default-mode*), *volume snapshots*, or *clone*, then run <<oracle-finalize,Oracle finalize>>. iSCSI and ASM are the same as <<provision-gcnv-iscsi-volumes,Step 2: Provision GCNV iSCSI volumes>>.
...
|===
| Step | Action
| 1 | Primary archivelog + force logging (<<primary-sys-password-and-dg-params,Primary: SYS password, password file, and DG parameters>>)
...
| 4 | On `oracdb2`: iSCSI, multipath, ASM mount (<<configure-iscsi-and-multipath,Configure Linux iSCSI and multipath for GCNV iSCSI volumes>>, <<partition-asm-backing-devices,Partition ASM backing devices on GCNV iSCSI volumes>>, and <<format-and-mount-u01,Format and mount `/u01` on the local GCNV iSCSI volume>>)
| 5 | Oracle finalize — recover to SCN, *MOUNTED* (<<oracle-finalize,Oracle finalize>>)
| 6 | SRL rebuild (<<standby-redo-log-files,Standby redo log files>>), MRP, broker (<<broker-fsfo-observer,Step 5: Configure Data Guard Broker, FSFO, and Observer>>)
|===
...
After convert, *rebuild standby redo logs* (replicated controlfile still has `+DATA/ORCL/…` SRL paths — causes `ORA-19527` / `ORA-16086` on primary). See <<standby-redo-log-files,Standby redo log files>>.
...
=== Enable flashback on the standby and start managed recovery
...
=== Enable redo shipping on the primary

`LOG_ARCHIVE_DEST_2` was deliberately set to `DEFER` in <<standby-init-ora-and-nomount,Standby: minimal init.ora pfile, password file, NOMOUNT>> so you didn’t see continuous `ORA-12154` errors while the standby was being created. Now switch it to `ENABLE` and force a log switch so the first round of redo ships immediately:
...
If `dest_2` shows `ORA-12154`, bounce the primary. After <<enable-broker-on-both-databases,Enable the broker on both databases>>, manage transport via DGMGRL.

[[verify-target-dg-state]]
=== Verify the target Data Guard state
...
== Step 5: Configure Data Guard Broker, FSFO, and Observer
...
On the *primary database host*, connect with OS authentication (the Observer wallet from <<start-observer-systemd,Start the Observer as a systemd unit>> is not required on DB hosts):
...
Run `VALIDATE DATABASE` immediately — it surfaces orphaned datafiles, missing SRLs, redo apply not running, and other common gotchas before you attempt a switchover. Any `WARNING` or non-NULL `ERROR` must be fixed before <<configure-fsfo-properties-and-enable,Configure FSFO properties and enable>>.
...
=== Confirm flashback (required for FSFO auto-reinstate)
...
Expected: Threshold 30 seconds, Target `orcls`, Observer none until <<start-observer-systemd,Start the Observer as a systemd unit>>.

[[install-instant-client-on-observer]]
=== Install Oracle Instant Client on the Observer host
...
=== Start the Observer as a systemd unit
