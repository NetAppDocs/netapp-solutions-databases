...
<<example-configuration,Example configuration used in this guide>>
...
<<provision-compute-engine,Provision Compute Engine>>

<<provision-gcnv-iscsi-volumes,Provision GCNV iSCSI volumes>>

<<install-oracle-software,Install Oracle software>>

<<create-primary-database,Create the primary database>>
...
<<create-standby-database,Create the standby database>> (through <<gcnv-standby-initialization,Step 3: GCNV standby initialization>> only)
...
<<create-standby-database,Create the standby database>>

<<broker-fsfo-observer,Configure Data Guard Broker, FSFO, and Observer>>
|===
