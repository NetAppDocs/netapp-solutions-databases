...
| Yes
| No
| <<prerequisites,Prerequisites>> through <<provision-gcnv-iscsi-volumes,Provision GCNV iSCSI volumes>>, <<install-oracle-db-home,Install Oracle Database 26ai on each DB host>>, and <<create-primary-database,Create the primary database>>
| Snapshot clone
...
image::oracle-26ai-ha-on-gcnv-3-data-paths.png[alt="Oracle 26ai HA on GCNV - reference architecture"]
...
| 6 | SRL rebuild (<<standby-redo-log-files,Standby redo log files>>), MRP, broker (<<broker-fsfo-observer,Configure Data Guard Broker, FSFO, and Observer>>)
|===
