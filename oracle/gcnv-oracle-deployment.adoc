...
Substitute the GCE internal IP addresses (visible in the *Compute Engine → VM instances* list, _Internal IP_ column).
...
[NOTE]
====
*Security posture (OL 10):* The commands below set SELinux to permissive and disable `firewalld`. This is a minimal lab posture only. See <<appendix-b-security-posture,Appendix B>> for hardened SELinux and firewall configuration.
====
...
[NOTE]
====
`LOG_ARCHIVE_DEST_2` was deliberately set to `DEFER` in <<standby-init-ora-and-nomount,Step 2: Standby: minimal init.ora pfile, password file, NOMOUNT>> to suppress continuous `ORA-12154` errors during standby creation. Enable it now that the standby is ready.
====
