...
Before you begin, ensure you have completed the following requirements:
...
NOTE: New Google Cloud users might be eligible for free trial credits.

== Objectives

This setup guide covers the following tasks:

* Setting up Compute Engine instances and NetApp storage volumes
* Configuring SQL Server on both nodes
* Setting up Windows Server Failover Cluster
* Configuring cluster quorum with file share witness
* Creating and configuring SQL Server availability groups
* Setting up Distributed Network Name (DNN) for listener access
...
Use the Google Cloud Pricing Calculator to generate a cost estimate based on your compute and storage requirements. The example configuration uses N4-SKU compute instances and NetApp Flex service level storage for the SQL Server Always On availability group setup.
...
To create two compute instances (sqlnode1, sqlnode2) with SQL Server 2022 Enterprise preinstalled on Windows Server 2025:
...
. For *Network service tier*, select *Premium* for mission-critical workloads or *Standard* to optimize costs.
...
Create a host group containing the iSCSI initiators from both SQL nodes:
...
image::gcnv-sql-aoag-001.png[Host group creation in Google Cloud console]

=== Create storage pool

Create a storage pool with appropriate capacity and performance:
...
image::gcnv-sql-aoag-002.png[Storage pool configuration]

=== Create volumes

Create volumes for data, logs, temp, and backup. Run the following command for each volume type:
...
image::gcnv-sql-aoag-003.png[Volume creation for SQL Server]
...
+
image::gcnv-sql-aoag-004.png[iSCSI Initiator - Discover Portal]
...
. Click *OK* to connect.
+
image::gcnv-sql-aoag-005.png[Connect to iSCSI target]
...
+
image::gcnv-sql-aoag-006.png[MPIO configuration for iSCSI devices]
...
. Create directory structure for SQL Server:
+
[source,powershell]
----
...
----
+
image::gcnv-sql-aoag-007.png[Disk Management - Initialize and format volumes]
...
=== Update default SQL Server paths
...
+
image::gcnv-sql-aoag-008.png[SQL Server Configuration Manager - Startup Parameters]
...
+
image::gcnv-sql-aoag-009.png[Failover Cluster - Validation tests]

. Provide a cluster name (for example, `sqlcluwest1`).
. Complete the cluster creation.
+
image::gcnv-sql-aoag-010.png[Failover Cluster creation]
...
.. Browse to or create a path for the shared folder (for example, `C:\ClusterQuorum`)
.. Verify the share name and path
.. Select *Customize permissions* and click *Custom*
.. Add the cluster account (for example, `sqlcluwest1$`) with *Full Control*
.. Click *OK* to save permissions
.. Complete the wizard
+
image::gcnv-sql-aoag-011.png[File share witness permissions]
...
image::gcnv-sql-aoag-012.png[Cluster quorum configuration with file share witness]
...
+
image::gcnv-sql-aoag-013.png[Enable Always On High Availability]

== Create test database

Create a test database on the first SQL Server instance:
...
Before creating the availability group, take full and transaction log backups:
...
=== Restore backups on secondary replica
...
Create the availability group with synchronous commit and automatic failover:
...
ALTER AVAILABILITY GROUP sqlagwest1 JOIN;
GO

-- Join the database
ALTER DATABASE MyDB1 SET HADR AVAILABILITY GROUP = sqlagwest1;
GO
----

IMPORTANT: Ensure firewall port 5022 is allowed on both SQL nodes.

image::gcnv-sql-aoag-014.png[Create availability group]

image::gcnv-sql-aoag-015.png[Add replicas to availability group]

image::gcnv-sql-aoag-016.png[Availability group configuration complete]
...
The list of possible owners for the DNN resource should match the list of possible owners for the SQL Server availability group resource.

image::gcnv-sql-aoag-017.png[DNN resource possible owners configuration]
...
Verify availability group configuration and test failover:

.Verify configuration
[source,sql]
----
-- Run on any replica to verify AG primary and synchronization mode
...
Ensure both nodes show `SYNCHRONOUS_COMMIT` for availability mode and `AUTOMATIC` for failover mode.

image::gcnv-sql-aoag-018.png[Verify availability group configuration]

.Initiate failover
Run the following command on the secondary replica to initiate failover:
...
In SSMS, expand the availability group node, right-click *Always On High Availability*, and select *Show Dashboard*. The dashboard should display both nodes with healthy status and confirm the failover.

image::gcnv-sql-aoag-019.png[Availability group failover verification]

image::gcnv-sql-aoag-020.png[Always On High Availability dashboard]
...
Refer to Google Cloud documentation for detailed steps on deleting resources.
