...
- link:https://docs.cloud.google.com/compute/docs/instances/sql-server/configure-availability#before-you-begin[Before you begin^]
- link:https://docs.cloud.google.com/compute/docs/instances/sql-server/configure-availability#prepare_your_project_and_network[Prepare your project and network^]
- link:https://docs.cloud.google.com/compute/docs/instances/sql-server/configure-availability#create_firewall_rules[Firewall rules^]
- link:https://docs.cloud.google.com/netapp/volumes/docs/plan-and-prepare/workflow[Set up NetApp volumes^]
...
This tutorial uses billable components of Google Cloud, including link:https://console.cloud.google.com/compute/overview?[Compute Engine instances^] and link:https://console.cloud.google.com/netapp/volumes?[Google Cloud NetApp Volumes^] storage.

Use the link:https://cloud.google.com/products/calculator?dl=CjhDaVF4T1dJeU5XRTBZUzFtTmpjMkxUUTRZVGd0WVdOalpDMHhNMkpsTVRVMU9UaG1OVFlRQVE9PRAqGiRBNzVCRUZBMC0zMjBGLTQ1OTctQjNFQy1BQzk5OEYxMTg1NDA[Pricing Calculator^] to generate a cost estimate based on your compute and storage requirements. The example configuration uses N4-SKU compute instances and NetApp Flex service level storage for the SQL Server Always On availability group setup.
...
NOTE: This example uses `cvsdemo` as the domain name. Replace `cvsdemo` with your actual domain name throughout this procedure.
...
.Steps
. In the Google Cloud console, go to the link:https://console.cloud.google.com/compute/instancesAdd[*Create an instance*] page.
+ 
Refer to the link:https://cloud.google.com/compute/docs/instances/create-start-instance[Google Cloud documentation^] for more information.
...
. In the *Networking* section, edit the network interface to select the correct VPC and subnet. If you have only one VPC network, it will be selected by default.
...
. On the *Features* page, select *Failover Clustering* and *Multipath I/O*.
...
. Press *Win+R* or use the Windows search bar to open `iscsicpl`.
. In the iSCSI Initiator Properties dialog, go to the *Configuration* tab.
. Copy the *Initiator Name* value and include it in the host group.
+
...
----
+
For more details, refer to link:https://cloud.google.com/netapp/volumes/docs/configure-and-use/host-groups/create-host-group[Create a host group^] documentation.
+
...
----
+
For more details, refer to link:https://cloud.google.com/netapp/volumes/docs/configure-and-use/storage-pools/create-storage-pool[Create a storage pool^] documentation.
+
. Replace the following values:
...
----
+
For more details, refer to link:https://cloud.google.com/netapp/volumes/docs/configure-and-use/volumes/create-volume[Create a volume^] documentation. 
+
. Replace the following values:
...
+
NOTE: Replace `CVSDEMO` with your actual domain name in the following SQL commands.
...
+
image::gcnv-sql-failover-confirm.png[Failover Cluster creation]
...
+
image::gcnv-sql-configure-dns.png[Configure DNN possible owners]
